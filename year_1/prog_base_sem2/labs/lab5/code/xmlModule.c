...
void xmlModule_listToXML(list_t *listObj, const char *xmlFilePath)
{
    ...
    for (int i = 0; i < list_getSize(listObj); i++)
    {
        ...
        directorNode = xmlNewChild(rootNode, NULL, (xmlChar *)"director", NULL);
        // Fill certain director info.
        xmlNewProp(directorNode, (xmlChar *)"name", (xmlChar *)director_getName(currentDirector));
        xmlNewProp(directorNode, (xmlChar *)"surname", (xmlChar *)director_getSurname(currentDirector));
