...
void server_analyzeGETRequest(http_request_t * request, socket_t * client, db_t *dbObj)
{
    ...
    else if (strstr(request->uri, "/api/directors?"))
    {
        ...
        sscanf(request->uri, "/api/directors?%s", requestToParse);
        // Get request values.
        int invVal = 0;
        double yearsVal = 0;
        char fS[10] = "\0", sS[10] = "\0", yearsValStr[10] = "\0";
        char firstSign = 'a', secondSign = 'a';
        int checkCode = sscanf(requestToParse, "inv%3s%d&years%3s%3s", fS, &invVal, sS, yearsValStr);
        yearsVal = atof(yearsValStr);
        if (checkCode == 4)
        {
            if (!strcmp(fS, "%3C"))
            {
        ...
        {
            puts("Wrong complex request.");
            http_sendHtml(client, "src/html/404page.html");
            return;
        }
        // Do request to database and get list of directors under certain request.
        list_t *listToXMLObj = database_filterDirectorsv2(dbObj, invVal, yearsVal, firstSign, secondSign);
        // Write this list to databaseRequest.xml
        xmlModule_listToXML(listToXMLObj, "src/data/dbRequest.xml");
        // Send .xml file to server.
        http_sendXML(client, "src/data/dbRequest.xml");
        // Free allocated memory - list
        list_delete(listToXMLObj);
    }
    else if (strstr(request->uri, "/api/directors/"))
    {
        int id = 0;
...
void server_analyzePOSTRequest(http_request_t * request, socket_t * client, db_t *dbObj)
{
    if (!strcmp(request->uri, "/new-director"))
    {
        ...
        database_insertDirector(dbObj, dirToAdd);
        server_updateDirectorsXML(dbObj);
...
void server_analyzeDELETERequest(http_request_t * request, socket_t * client, db_t *dbObj)
{
    if (strstr(request->uri, "/api/directors/"))
    {
        ...
        if (checkCode != 0 && id > 0)
        {
            database_deleteDirectorById(dbObj, id);
            server_updateDirectorsXML(dbObj);
            http_sendXML(client, "src/data/directors.xml");
        }
        else
...
{
    list_t *dirsList = database_getDirectorsList(dbObj);
    xmlModule_listToXML(dirsList, "src/data/directors.xml");
    free(dirsList);
}
