...
list_t *database_filterDirectorsv2(db_t *dbObj, int invVal, double yearsVal, char fS, char sS)
{
    list_t *dirsList = list_new();
    ...
    sqlite3_stmt *stmt = NULL;
    char sqlCommand[100];
    sprintf(sqlCommand, "SELECT * FROM Directors WHERE Investments %c ? AND Years %c ?;", fS, sS);
    int returnCode = 0;

    // Prepare SELECT statement.
    returnCode = sqlite3_prepare_v2(dbObj->db, sqlCommand, strlen(sqlCommand) + 1, &stmt, NULL);
    // Bind values.
    errorHandler_NotEqual(returnCode, SQLITE_OK, "Error binding first sign.");
    returnCode = sqlite3_bind_int(stmt, 1, invVal);
    errorHandler_NotEqual(returnCode, SQLITE_OK, "Error binding investments value.");
    returnCode = sqlite3_bind_double(stmt, 2, yearsVal);
    errorHandler_NotEqual(returnCode, SQLITE_OK, "Error binding years value.");
    ...
    while (TRUE)
    {
        ...
        {
            _directorFill(stmt, directorSet[directorSetIndex]);
            list_insertLast(dirsList, (void *)directorSet[directorSetIndex]);
            directorSetIndex++;
        }
    }
    returnCode = sqlite3_finalize(stmt);
    errorHandler_NotEqual(returnCode, SQLITE_OK, "Error destroying database statement.");
